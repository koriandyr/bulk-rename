...
# Run quality checks (must pass 10.00 pylint, 90% coverage)
...
- **Quality gates**: 90% coverage, 10.00 pylint, all tests pass
...
- Target 90% coverage. Prioritise tests that validate business logic, error
  handling, and data flow. Do not write tests solely to chase coverage — omit
  tests for code where the effort of writing a meaningful test exceeds the
  quality or safety benefit it provides (e.g. trivial getters, log-only paths,
  defensive branches that cannot realistically be reached). Use
  `# pragma: no cover` for entry-point blocks and any branch where the only
  way to exercise it would be through a brittle or low-value test.
...
1. EXIF `DateTimeOriginal` tag 36867 (images) — treated as UTC
2. Windows Property System `PKEY_Media_DateEncoded` (videos)
3. FFprobe `creation_time` (videos)
4. File modification time converted to UTC (fallback for images and videos)

**Note**: EXIF `DateTime` (tag 306) is intentionally skipped. It stores local time
written by editing software, not the camera capture time. Stamping it with UTC would
produce an incorrect `Z`-suffixed filename.
...
- `tests/test_bulk_rename.py`: Comprehensive tests (1275 lines, 90% coverage)
